...
export default function AdminUserManager({ adminId }: Props) {
  ...
  const inputCls =
    "w-full bg-white/5 border border-white/10 px-3 py-2 text-white text-sm outline-none focus:border-[#C9AB81]/50 placeholder:text-white/20";
  const labelCls =
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <p className="text-white/30 text-xs">{admins.length} administratori</p>
        <button
      ...
      {/* Form */}
      {showForm && (
        <div className="bg-white/[0.03] border border-white/[0.06] p-4 mb-4 space-y-3">
          ...
          {admins.map((a) => {
            ...
            return (
              <button
                key={a.id}
                onClick={() => startEdit(a)}
                className="w-full bg-white/[0.03] border border-white/[0.06] px-4 py-3 flex items-center justify-between text-left"
              >
                <div>
